...
const UI = {
  ...
  resetTabs() {
    ...
    if (firstTab) this.selectTab(firstTab, "الكل");
      ...
      // 🛒 فتح/إغلاق مودال الـ Checkout وتحديث بيانات الدفع
  openCheckout() {
    Cart.updateUI();
    
    // ربط الأرقام وعناوين التحويل ديناميكياً من ملف الإعدادات CONFIG إن وجدت
    const walletNumDisplay = document.getElementById("wallet-num-display");
    const instapayAddressDisplay = document.getElementById("instapay-address-display");

    if (walletNumDisplay && CONFIG.PAYMENT_METHODS?.VODAFONE?.number) {
      walletNumDisplay.textContent = CONFIG.PAYMENT_METHODS.VODAFONE.number;
    }
    if (instapayAddressDisplay && CONFIG.PAYMENT_METHODS?.INSTAPAY?.number) {
      instapayAddressDisplay.textContent = CONFIG.PAYMENT_METHODS.INSTAPAY.number;
    }

    // إعادة ضبط الاختيار الافتراضي على الكاش وإخفاء القوائم الأخرى عند فتح المودال مجدداً
    const defaultRadio = document.querySelector('input[name="payment-method"][value="cash"]');
    if (defaultRadio) {
  ...
  handlePaymentMethodChange() {
    ...
    // إظهار أو إخفاء الصناديق بناءً على القيمة المختارة بالـ HTML
    if (walletDetails) walletDetails.classList.toggle("hidden", selected !== "wallet");
    if (instapayDetails) instapayDetails.classList.toggle("hidden", selected !== "instapay");
    
    // تحديث الإستايل البصري للبطاقات لجعل البطاقة المختارة مميزة بحدود زرقاء وخلفية فاتحة
    document.querySelectorAll('input[name="payment-method"]').forEach(input => {
      const parentLabel = input.closest('label');
      if (parentLabel) {
        if (input.checked) {
          parentLabel.className = "flex flex-col p-3.5 bg-blue-50/50 border border-blue-500 rounded-xl cursor-pointer transition-all active:scale-98 ring-1 ring-blue-500/10";
        } else {
          parentLabel.className = "flex flex-col p-3.5 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer transition-all active:scale-98";
        }
      }
    });
  },

  // وظيفة النسخ الذكي والسرع بنقرة واحدة متوافقة مع الـ HTML الحالي
  copyPaymentNumber(type) {
    let numberText = "";
    let btnElement = null;

    if (type === 'wallet') {
      numberText = document.getElementById("wallet-num-display")?.textContent;
      btnElement = document.querySelector("#wallet-details button");
    } else if (type === 'instapay') {
      numberText = document.getElementById("instapay-address-display")?.textContent;
      btnElement = document.querySelector("#instapay-details button");
    }

    if (!numberText || !btnElement) return;

    const originalText = btnElement.textContent;
    navigator.clipboard.writeText(numberText).then(() => {
      btnElement.textContent = "تم نسخها! ✓";
      btnElement.className = "text-xs bg-emerald-600 text-white px-2 py-1 rounded-md font-bold active:scale-90 transition-transform";
      setTimeout(() => { 
        btnElement.textContent = originalText; 
        btnElement.className = type === 'wallet' ? "text-xs bg-red-600 text-white px-2 py-1 rounded-md font-bold active:scale-90 transition-transform" : "text-xs bg-teal-600 text-white px-2 py-1 rounded-md font-bold active:scale-90 transition-transform";
      }, 2000);
    }).catch(err => {
